    ...
    <div className="flex flex-row pt-10 pb-10 items-center w-full justify-center">
      <section className="p-5 w-full md:w-1/2 flex items-center justify-between md:justify-around">
        ...
        <section className=" w-44  md:w-52 flex flex-col items-center">
          <h1 className="font-semibold text-base p-2">
            {jsonData ? jsonData.plantName : "Error Collecting Data"}
          </h1>
          <p className="flex flex-row justify-around p-2 md:w-44">
            <img src="/star.png" alt="star" className="w-5 h-5" />{" "}
            {jsonData ? jsonData.rating : "_"} |
            <img src="/user.png" alt="image" className="w-5 h-5" />{" "}
            {jsonData ? jsonData.reviews : "_"} |{" "}
            <img src="/ready-stock.png" className="w-5 h-5" alt="image" />
            {jsonData ? jsonData.itemSold : "_"}
          </p>
          <p className="text-red-500  font-bold text-base">
            Price: ${jsonData ? jsonData.price : "_"}
          </p>

          <section className="flex flex-row justify-between w-28">
            <p
              className=" text-slate-50 w-7 h-7 text-center items-center bg-red-700 hover:bg-red-400"
              onClick={subtract}
            >
              -
            </p>
          ...
          <button
            onClick={addToCart}
            className="w-32 md:w-40 p-3 bg-red-500 text-slate-50 text-center hover:bg-emerald-500 mt-5"
          >
            Add to cart
          </button>
        </section>
